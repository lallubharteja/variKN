...
template <typename KT, typename ICT>
VarigramProbsFull_t<KT, ICT>::~VarigramProbsFull_t() {
  delete m_problm;
  delete m_initial_ng;
  delete m_data;
}
...
template <typename KT, typename ICT>
void VarigramProbsFull_t<KT, ICT>::
initialize(std::string infilename, indextype hashsize, 
  ...
  m_problm = new ProbsLM_impl<KT, ICT>(infilename, vocabname, 1, datastorage_tmp, clhist, readprob, false, hashsize);
...
template <typename KT, typename ICT>
void VarigramProbsFull_t<KT, ICT>::grow(int iter2_lim) {
  ...
  while (iter2<iter2_lim) {
	...
	if (new_history.size()!=old_hist_size) {
	  ...
	  m_data->initialize_fast_search_lists_probs(new_history.size()+1, curref1, curref2);
	}
...
template <typename KT, typename ICT>
void VarigramProbsFull_t<KT, ICT>::prune() {
...
template <typename KT, typename ICT>
bool VarigramProbsFull_t<KT, ICT>::reestimate_with_history(std::vector<KT> &v) {
  ...
  m_data->fast_search_next(&v, &idx, &val, &prob);
  m_data->fast_search_next(NULL, &idx, &val, &prob);
  while (idx>=0) {
    m_new_c[idx]+=val;
    m_new_prob[idx]+=prob;
    new_c_sum += val;
...
template <typename KT, typename ICT>
double VarigramProbsFull_t<KT, ICT>::modify_model(
...
template <typename KT, typename ICT>
void VarigramProbsFull_t<KT, ICT>::write(FILE *out) {
